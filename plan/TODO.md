...
- 🟡 `#index` fixtures (§5.5) once the meta line lands in the spec.
...
## 5. `#index` fenceposts (parallel scan)
...
## 6. `#chart` chart suggestions

Spec: [`docs/implementation/charts.md`](../docs/implementation/charts.md) (normative) + [`docs/charts.md`](../docs/charts.md) (guide). Reference renderer target: [asciicharts](https://github.com/boligolov/asciicharts).
...
- Sorted-id skip index (id on the fencepost) — not part of §5; add later if we want `WHERE id=` without a scan.
