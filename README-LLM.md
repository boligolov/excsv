...
Readers conforming to v0.2 MUST follow the existing forward-compat rules when encountering them: unknown header keys → ignored (per HEADER LINE), unknown `#`-prefixed meta lines → ignored (per META LINES). No special handling is required.
