...
- Recommended order: `#@` metadata first, then `#column`, then `#$` SQL, then `#%` aggregations. Order within each group does not matter, except where stated (e.g. `#$ddl` execution order). Parsers **MUST** accept any order. `##` comments **MAY** appear anywhere in the meta block.
...
See dedicated pages: [File metadata](file-metadata.md), [Columns](columns.md), [SQL](sql.md), [Aggregations](aggregations.md).
