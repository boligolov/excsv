...
4. If csvw payload: write "#csvw: <json>".
5. Write "#$<verb>[-<dialect-suffix>]: <statement>" lines preserving the file's original insertion order. Multiple entries with the same key are allowed and MUST be emitted in order.
...
Recommended meta order (steps 2–6): `#@` → `#column` → `#csvw` → `#$` → `#%`. See [meta-lines.md](meta-lines.md).
