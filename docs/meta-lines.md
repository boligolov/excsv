...
| `#csvw: ...` | Embedded W3C CSVW metadata | `#csvw: {"tableSchema": ...}` |
| `## ...` | A plain human comment — free text, ignored by tools | `## TODO: drop legacy status values` |

A few conveniences:

- **Order is up to you.** A common, readable order is `#@` first, then `#column`, then `#csvw`, `#$`, and `#%` — but tools accept any order. (The one place order carries meaning is a sequence of `#$ddl` statements, which run top to bottom.)
...
Each kind has its own page: [File metadata](file-metadata.md), [Columns](columns.md), [SQL](sql.md), [Aggregations](aggregations.md), [CSVW](csvw.md).
